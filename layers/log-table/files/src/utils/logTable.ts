...
function clampTerminalWidth(width: number, cfg: LogTableSettings) {
  return Math.max(cfg.minTerminalWidth, Math.min(width, cfg.maxTerminalWidth));
}

export function cellDisplayWidth(value: unknown) {
  if (value == null) return 0;
  return stringWidth(String(value));
}

export function truncateCell(value: unknown, maxWidth: number) {
...
function tableFrameOverhead(columnCount: number) {
  return columnCount + 1;
}

function toColWidth(contentWidth: number) {
...
export function computeColumnWidths(
  ...
  const specs = columns.map((col) => {
    ...
    const contentWidth = rows.reduce(
      (max, row) => Math.max(max, cellDisplayWidth(row[col.key])),
      headerWidth,
    );
  ...
  const widths: Record<string, number> = Object.fromEntries(specs.map((s) => [s.key, s.minWidth]));
