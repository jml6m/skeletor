...
- Keep changes scoped to the requested task. Avoid drive-by refactors of the generator or unrelated templates.
- When adding a template, follow the manifest contract in `AGENTS.md` (`template.json` with `verifyCommands`) — the test suite enforces it.
