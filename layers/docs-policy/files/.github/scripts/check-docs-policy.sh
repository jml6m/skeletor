...
if [ "${#offenders[@]}" -gt 0 ]; then
  ...
  printf '  - %s\n' "${offenders[@]}" >&2
  echo >&2
  echo "Markdown files are locked to an exact allowlist to keep" >&2
  ...
  echo "     (protect .github/ with CODEOWNERS so additions get a review from @{{REPO_OWNER}})." >&2
  exit 1
fi
