...
- Test the generator behavior (token substitution, `.tmpl` suffix stripping, `--yes` non-interactivity), not the runtime of every generated stack.
- Keep generation deterministic: same inputs produce the same tree. Do not introduce time/random-dependent output.
- `--yes` must remain fully non-interactive — a regression here breaks scripting/CI and must be covered.
